...
function on_xinput_get_state(retval, user_index, state)
	...
	if is_hands_active and hands.exists() then
		-- handleInputForHands processes both hands in one call (v2 API).
		-- Passing nil for both attachments = neither hand is holding a weapon.
		-- This also disables the automatic input handler so input is not processed twice.
		hands.handleInputForHands(state, nil, nil)
	end
end